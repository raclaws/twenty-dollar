...
export const SetupPage = observer(function SetupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setValidationError('');
    setSubmitting(true);
    const success = await authStore.setup(name, email, password);
    if (success) {
      // Trigger server hydration after setup
      runInAction(() => {
    ...
    <div className="flex items-center justify-center min-h-screen p-4">
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-zinc-400">Set up your first account</p>
        </div>
