...
export function parseDate(raw: string, contextYear?: number): string {
  ...
  if (m) {
    ...
    if (a > 12) return `${m[3]}-${m[2]}-${m[1]}`  // DD/MM/YYYY (day > 12)
    if (b > 12) return `${m[3]}-${m[1]}-${m[2]}`  // MM/DD/YYYY (day > 12)
    return `${m[3]}-${m[2]}-${m[1]}`              // ambiguous, assume DD/MM/YYYY
  }
...
export function parseAmount(raw: string, direction: Token | null, amountNegative?: boolean): number {
  ...
  if (lastComma > lastDot) {
    ...
    // Check if last dot is a decimal or thousands separator
    const afterDot = s.slice(lastDot + 1)
    if (afterDot.length === 3 && s.indexOf('.') !== lastDot) {
      // All dots are thousands separators (e.g. 1.234.567)
      s = s.replace(/\./g, '')
    } else {
      s = s.replace(/,/g, '')
    }
  ...
  // Remove any remaining dashes for parsing (we already captured the sign)
  s = s.replace(/-/g, '')
