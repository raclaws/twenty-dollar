...
export function mapColumns(transactions: RawTransaction[]): MappedTransaction[] {
  ...
  // Detect debit/credit column split by position clustering
  const clusters = clusterPositions(transactions)
  let debitCenter: number | null = null
  let creditCenter: number | null = null

  // If 3 clusters: debit, credit, balance (by position left to right)
  // If 2 clusters and one is balance: the other is amount (sign from direction or prefix)
  if (clusters.length === 3) {
    // Rightmost is balance, middle two are debit/credit
    // Debit: most transactions have amounts here, and they're expenses
    // Credit: fewer transactions, they're income
    // Heuristic: the cluster where fewer transactions have amounts is credit (income is rarer)
  ...
  return transactions.map((tx, txIdx) => {
    ...
    if (tx.moneyTokens.length === 0) {
      ...
      // First priority: if one token has '-' prefix on its line, that's the amount
      ...
      if (prefixedToken) {
        ...
        // Smallest value is transaction amount, largest is running balance
    ...
    // Fallback: check '-' prefix for amount token
    if (!amountNegative) {
      for (const line of tx.lines) {
