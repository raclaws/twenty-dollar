...
export function findRegion(lines: TokenizedLine[]): Region {
  ...
  // Find first and last lines that look like transaction starts (DATE present)
  let first = -1
  let last = -1
  ...
  // Extend past the last DATE to include continuation lines (text, money) until noise/empty
